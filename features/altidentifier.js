...
module.exports = {
    ...
    run: async(client) => {
        client.on('guildMemberAdd', async (member) => {
            ...
            const aichannel = guild.channels.cache.get(await client.gData.get(`${guild.id}:aichannel`))
            const aitime = await client.gData.get(`${guild.id}:aitime`)
            if(!aichannel) return
            const createdAt = new Date(member.user.createdAt).getTime();
            const difference = Date.now() - createdAt;

            if(difference < ms(aitime)) {
                member.send(`This server is protected by an alt identifier. You have been kicked because you have been identified as an alt or your because your account is too new. Join back when your account is over ${ms(ms(aitime), {long: true})}`)
                member.kick()
